...
func Restore(id string, progress chan int) ([]byte, error) {
	progress <- 1
	...
	if post.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("invalid server response: %s (%d)\nfor revision %s", post.Status, post.StatusCode, id)
	}
	progress <- 1
	...
	if revision.Hash != hex.EncodeToString(decHash[:]) {
		return nil, fmt.Errorf("decrypted data is corrupted")
	}
	progress <- 1
	conf := string(dec)
	if !strings.Contains(conf, "<pfsense>") {
		return nil, fmt.Errorf("cleartext data seems invalid")
	}
	progress <- 1
